#include "../src/models/qwen2/qwen2.hpp"
#include "../src/models/qwen2/qwen2_session.hpp"
#include "../src/llaisys/llaisys_tensor.hpp"
#include "../src/tensor/tensor.hpp"

#include <cmath>
#include <cstdint>
#include <iostream>
#include <memory>
#include <sstream>
#include <stdexcept>
#include <string>
#include <vector>

using llaisys::models::Qwen2Model;
using llaisys::models::Qwen2Session;
using llaisys::Tensor;

namespace {

void expect(bool cond, const std::string &message) {
    if (!cond) {
        throw std::runtime_error(message);
    }
}

template <typename Fn>
void expect_invalid_argument_silent(Fn &&fn, const std::string &message) {
    std::ostringstream sink;
    auto *old_buf = std::cerr.rdbuf(sink.rdbuf());
    bool caught = false;
    try {
        fn();
    } catch (const std::invalid_argument &) {
        caught = true;
    }
    std::cerr.rdbuf(old_buf);
    expect(caught, message);
}

void load_tensor(llaisysTensor_t tensor, const std::vector<float> &values) {
    expect(tensor != nullptr, "load_tensor: null tensor");
    expect(tensor->tensor->dtype() == LLAISYS_DTYPE_F32, "load_tensor: expected f32 tensor");
    expect(tensor->tensor->numel() == values.size(), "load_tensor: size mismatch");
    tensor->tensor->load(values.data());
}

void fill_tensor(llaisysTensor_t tensor, float value) {
    std::vector<float> values(tensor->tensor->numel(), value);
    load_tensor(tensor, values);
}

std::vector<float> identity_matrix(size_t rows, size_t cols) {
    std::vector<float> values(rows * cols, 0.0f);
    const size_t diag = std::min(rows, cols);
    for (size_t i = 0; i < diag; ++i) {
        values[i * cols + i] = 1.0f;
    }
    return values;
}

void initialize_test_weights(Qwen2Model &model, size_t nlayer, size_t voc, size_t hs, size_t nkvh, size_t dh, size_t di) {
    auto &weights = model.weights();

    std::vector<float> embed(voc * hs, 0.0f);
    for (size_t token = 0; token < voc; ++token) {
        for (size_t i = 0; i < hs; ++i) {
            embed[token * hs + i] = std::sin(static_cast<float>((token + 1) * (i + 1))) * 0.25f;
        }
    }
    load_tensor(weights.in_embed, embed);
    load_tensor(weights.out_embed, embed);
    fill_tensor(weights.out_norm_w, 1.0f);

    for (size_t layer = 0; layer < nlayer; ++layer) {
        fill_tensor(weights.attn_norm_w[layer], 1.0f);
        load_tensor(weights.attn_q_w[layer], identity_matrix(hs, hs));
        fill_tensor(weights.attn_q_b[layer], 0.0f);

        std::vector<float> k_w(nkvh * dh * hs, 0.0f);
        std::vector<float> v_w(nkvh * dh * hs, 0.0f);
        for (size_t row = 0; row < nkvh * dh; ++row) {
            k_w[row * hs + row] = 1.0f;
            v_w[row * hs + row] = 1.0f;
        }
        load_tensor(weights.attn_k_w[layer], k_w);
        load_tensor(weights.attn_v_w[layer], v_w);
        fill_tensor(weights.attn_k_b[layer], 0.0f);
        fill_tensor(weights.attn_v_b[layer], 0.0f);

        load_tensor(weights.attn_o_w[layer], identity_matrix(hs, hs));
        fill_tensor(weights.mlp_norm_w[layer], 1.0f);
        fill_tensor(weights.mlp_gate_w[layer], 0.0f);
        fill_tensor(weights.mlp_up_w[layer], 0.0f);
        fill_tensor(weights.mlp_down_w[layer], 0.0f);
    }
}

std::unique_ptr<Qwen2Model> make_test_model(size_t maxseq = 32, size_t voc = 32) {
    LlaisysQwen2Meta meta{};
    meta.dtype = LLAISYS_DTYPE_F32;
    meta.nlayer = 1;
    meta.hs = 4;
    meta.nh = 2;
    meta.nkvh = 1;
    meta.dh = 2;
    meta.di = 4;
    meta.maxseq = maxseq;
    meta.voc = voc;
    meta.epsilon = 1e-6f;
    meta.theta = 10000.0f;
    meta.end_token = static_cast<int64_t>(voc - 1);
    int device_ids[1] = {0};
    auto model = std::make_unique<Qwen2Model>(meta, LLAISYS_DEVICE_CPU, device_ids, 1);
    initialize_test_weights(*model, meta.nlayer, meta.voc, meta.hs, meta.nkvh, meta.dh, meta.di);
    return model;
}

LlaisysQwen2SamplingParams deterministic_sampling() {
    LlaisysQwen2SamplingParams params{};
    params.temperature = 1.0f;
    params.top_k = 1;
    params.top_p = 0.0f;
    params.seed = 1;
    return params;
}

void test_incremental_matches_monolithic_prefill() {
    auto model_single = make_test_model();
    auto model_split = make_test_model();
    const auto params = deterministic_sampling();
    Qwen2Session session_single(*model_single);
    Qwen2Session session_split(*model_split);

    const int64_t token_single = session_single.infer({1, 2, 3}, params);
    session_split.infer({1, 2}, params);
    const int64_t token_split = session_split.infer({3}, params);

    expect(token_single == token_split, "incremental paged infer must match monolithic infer");
}

void initialize_test_weights_c(struct LlaisysQwen2Model *model, size_t nlayer, size_t voc, size_t hs, size_t nkvh, size_t dh, size_t di) {
    auto *weights = llaisysQwen2ModelWeights(model);

    std::vector<float> embed(voc * hs, 0.0f);
    for (size_t token = 0; token < voc; ++token) {
        for (size_t i = 0; i < hs; ++i) {
            embed[token * hs + i] = std::sin(static_cast<float>((token + 1) * (i + 1))) * 0.25f;
        }
    }
    load_tensor(weights->in_embed, embed);
    load_tensor(weights->out_embed, embed);
    fill_tensor(weights->out_norm_w, 1.0f);

    for (size_t layer = 0; layer < nlayer; ++layer) {
        fill_tensor(weights->attn_norm_w[layer], 1.0f);
        load_tensor(weights->attn_q_w[layer], identity_matrix(hs, hs));
        fill_tensor(weights->attn_q_b[layer], 0.0f);

        std::vector<float> k_w(nkvh * dh * hs, 0.0f);
        std::vector<float> v_w(nkvh * dh * hs, 0.0f);
        for (size_t row = 0; row < nkvh * dh; ++row) {
            k_w[row * hs + row] = 1.0f;
            v_w[row * hs + row] = 1.0f;
        }
        load_tensor(weights->attn_k_w[layer], k_w);
        load_tensor(weights->attn_v_w[layer], v_w);
        fill_tensor(weights->attn_k_b[layer], 0.0f);
        fill_tensor(weights->attn_v_b[layer], 0.0f);

        load_tensor(weights->attn_o_w[layer], identity_matrix(hs, hs));
        fill_tensor(weights->mlp_norm_w[layer], 1.0f);
        fill_tensor(weights->mlp_gate_w[layer], 0.0f);
        fill_tensor(weights->mlp_up_w[layer], 0.0f);
        fill_tensor(weights->mlp_down_w[layer], 0.0f);
    }
}

void test_compat_infer_still_works() {
    LlaisysQwen2Meta meta{};
    meta.dtype = LLAISYS_DTYPE_F32;
    meta.nlayer = 1;
    meta.hs = 4;
    meta.nh = 2;
    meta.nkvh = 1;
    meta.dh = 2;
    meta.di = 4;
    meta.maxseq = 32;
    meta.voc = 32;
    meta.epsilon = 1e-6f;
    meta.theta = 10000.0f;
    meta.end_token = 31;
    int device_ids[1] = {0};
    auto *c_model = llaisysQwen2ModelCreate(&meta, LLAISYS_DEVICE_CPU, device_ids, 1);
    initialize_test_weights_c(c_model, meta.nlayer, meta.voc, meta.hs, meta.nkvh, meta.dh, meta.di);

    auto session_model = make_test_model();
    Qwen2Session session(*session_model);
    const auto params = deterministic_sampling();

    int64_t prompt[] = {1, 2, 3};
    const int64_t via_model = llaisysQwen2ModelInfer(c_model, prompt, 3, &params);
    const int64_t via_session = session.infer({1, 2, 3}, params);
    expect(via_model == via_session, "compat infer should delegate to the same session semantics");
    llaisysQwen2ModelDestroy(c_model);
}

void test_exact_full_prefix_cache_bootstrap() {
    auto model = make_test_model(64, 64);
    const auto params = deterministic_sampling();
    Qwen2Session session(*model);

    std::vector<int64_t> prompt(16);
    for (size_t i = 0; i < prompt.size(); ++i) {
        prompt[i] = static_cast<int64_t>(i + 1);
    }

    const int64_t first = session.infer(prompt, params);
    session.reset();
    const int64_t second = session.infer(prompt, params);

    expect(first == second, "exact full-block cached prompt should reproduce the same next token");
}

void test_tensor_view_rejects_oversized_shape() {
    auto tensor = Tensor::create({8}, LLAISYS_DTYPE_F32, LLAISYS_DEVICE_CPU, 0);
    expect_invalid_argument_silent(
        [&]() { (void)tensor->view({9}); },
        "Tensor::view should reject shapes that exceed backing storage");
}

} // namespace

int main() {
    test_incremental_matches_monolithic_prefill();
    test_compat_infer_still_works();
    test_exact_full_prefix_cache_bootstrap();
    test_tensor_view_rejects_oversized_shape();
    std::cout << "qwen2_paged_test passed" << std::endl;
    return 0;
}
